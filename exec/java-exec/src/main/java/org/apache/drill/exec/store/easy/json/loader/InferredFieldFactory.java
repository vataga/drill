...
//      }
      if (loader.options().unionEnabled) {
//        final UnionVector vector = new UnionVector(fieldDefn.schemaForUnion(), allocator(), null);
        UnionWriterImpl unionWriter = new UnionWriterImpl(fieldDefn.schemaFor(scalarTypeFor(fieldDefn), true));
//        unionWriter.bindShim(new EmptyListShim());
//        unionWriter.bindShim(new SimpleListShim());
        ObjectWriter fieldWriter = fieldDefn.fieldWriterFor(fieldDefn.schemaForUnion());
        return variantParserFor(fieldWriter.variant());
      }
      if (valueDef.type().isObject()) {
        return objectParserFor(fieldDefn);
      } else {
        return scalarParserFor(fieldDefn, valueDef.isArray());
      }
    } else if (valueDef.dimensions() == 1) {
      if (loader.options().unionEnabled) {
...
//        return variantParserFor(fieldWriter.variant());
        ObjectWriter fieldWriter = fieldDefn.fieldWriterFor(fieldDefn.schemaForUnion());
        return variantParserFor(fieldWriter.variant());
//        return variantParserFor(new UnionWriterImpl(fieldDefn.schemaFor(scalarTypeFor(fieldDefn), valueDef.isArray())));
      }
      if (valueDef.type().isObject()) {
        return objectArrayParserFor(fieldDefn);
      } else {
        return scalarArrayParserFor(scalarParserFor(fieldDefn, valueDef.isArray()));
      }
    } else { // 2+ dimensions
      if (valueDef.type().isObject()) {
...
//    } else if(loader.options().unionEnabled) {
//      return variantParserFor(new UnionWriterImpl(fieldDefn.schemaFor(scalarTypeFor(fieldDefn), isArray)));
    } else {
